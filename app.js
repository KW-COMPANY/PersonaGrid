 ...
 * URLベース:
 * https://personagrid.gmo-k-watanabe.workers.dev/
 * ============================================================
 */
...
function renderResult(data) {
  ...
  const personaHTML = `
    <h2>シェア分類</h2>
    <p>${data.shareType || currentLevel.name}</p>
    ...
    <h2>セグメント提案</h2>
    <div>
      ${markdownToHTML(data.segment || "データなし")}
    </div>
  `;
...
function renderGauge(
  ...
  const gaugeBar =
    document.getElementById("gaugeBar");
   ...
   * ========================================================
   */
  gaugeBar.querySelectorAll(".gauge-markers")
...
function renderClassificationBadges(data) {
  ...
  const badges = [
    {
      key: "業界",
      value: data.industry || "未分類"
    },
    {
      key: "シェア分類",
      value: data.shareType || "未分析"
    },
    {
      key: "ターゲット",
      value: data.targetSegment || "未設定"
    },
    {
      key: "競合状況",
      value: data.competitors || "未設定"
    }
  ];
